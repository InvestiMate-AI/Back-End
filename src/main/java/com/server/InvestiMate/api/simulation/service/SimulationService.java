...
public class SimulationService {
    ...
    public List<Map<String, Object>> createSimulation2(GetSimulation2Request getSimulation2Request) {
        try {
            // JSON 데이터로 변환
            ObjectMapper mapper = new ObjectMapper();
            String jsonData = mapper.writeValueAsString(getSimulation2Request);

            // Python 스크립트 호출
            ProcessBuilder processBuilder = new ProcessBuilder(
                    "python3",
                    "/app/feedback/feedback.py",
                    "get_simulation2_result",  // 호출할 메서드 이름
                    jsonData  // JSON 데이터
            );
            Process process = processBuilder.start();
